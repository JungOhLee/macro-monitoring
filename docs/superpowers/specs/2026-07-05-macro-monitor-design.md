...
**Amendment 2026-07-06 (Phase 4 Task 5 follow-up — `ALPHAVANTAGE_KEY` activated; free-tier endpoint fix):** The GitHub Actions secret was activated, and the first live CI runs surfaced two problems the dormant-path unit tests couldn't catch. First, `TIME_SERIES_DAILY_ADJUSTED` (used for RSP/SPY above) is in fact a **premium** Alpha Vantage endpoint ("this is a premium API function" per Alpha Vantage's own docs) — every call returned an `Information` soft-failure, and both series silently fell back to Yahoo (which then 429s from Actions runners, per the existing staleness-budget mitigation). `_fetch_equity` now calls the free-tier `TIME_SERIES_DAILY` with `outputsize=compact` (latest ~100 observations — sufficient, since `run_ingest` merges the fetch onto the committed deep history rather than replacing it) and parses the raw `"4. close"` field instead of `"5. adjusted close"` (which doesn't exist on this endpoint anyway). The raw-close choice is deliberate, not just a premium-tier workaround: `pipeline/ingest/yahoo.py` stores `indicators.quote[0].close`, Yahoo's own raw/unadjusted close, so an adjusted-close Alpha Vantage observation would have spliced inconsistently onto Yahoo-sourced history the first time a dividend or split occurred between the two sources' observations — matching raw-to-raw avoids that latent inconsistency entirely. Second, `_get_json`'s soft-failure raise was upgraded to include a key-scrubbed excerpt of the failure message (`str(payload[bad_key]).replace(api_key, "***")[:200]`), still raised outside any `except` block so `__context__` stays `None` — this is what let us see the premium-endpoint message safely in CI logs in the first place, and is expected to do the same for BTC's still-unexplained `DIGITAL_CURRENCY_DAILY` `Information` response (`DIGITAL_CURRENCY_DAILY` itself is unchanged; only the diagnostic visibility improved).
